...
function unclaimed_job_more_info(title, job_location, date, start_time, end_time, pay, description, skills)
{
    ...
    '<div class="gray-background">' +
        ...
        '</div>' +
        `<button class="button-purple" onclick="window.location.href = 'employee.html';"> Remove </button> <button class="button-gray" onclick="delete_popup();"> Cancel </button>` +
        '</div>' +
    '</div>';
    const popup_div = document.getElementById("popup_div");
    popup_div.insertAdjacentHTML("afterBegin", popup_str);
...
function print_job_card(job)
{
    ...
    `<div class='job-card'>` +
        ...
        job.description +
        `<br>` +
        `<button class="button-purple" onclick="window.location.href = 'employee.html';"> Remove </button>` + //This is 100% wrong functionality
